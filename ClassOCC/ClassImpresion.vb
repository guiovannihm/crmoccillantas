...
Public Class ClassImpresion
    ...
    Public WriteOnly Property aLogo As String
        Set(value As String)
            lg = context.Server.MapPath(value)
        End Set
    End Property
    ...
    Public Sub cGENERAR_PDF()
        ...
        If enc IsNot Nothing Then
            ...
            tbenc.HorizontalAlignment = Element.ALIGN_LEFT
        ...
        If cl IsNot Nothing Then
            ...
            DOC.Add(tbc)
        End If
        If dsti.Rows.Count > 0 Then
            Dim tbc As PdfPTable = New PdfPTable(dsti.Columns.Count)
            tbc.WidthPercentage = 100

            For Each col As DataColumn In dsti.Columns
                Dim clclien As PdfPCell = New PdfPCell(New Phrase(col.ColumnName, _negrillaFont))
